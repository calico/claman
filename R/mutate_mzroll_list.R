...
#'   a \code{norm_peak_varname} variable added to measurements
#'
#' @examples
#'
#' normalize_peaks(
...
median_polish_predict_dilutions <- function(mzroll_list,
  ...
  updated_samples <- updated_samples %>%
    ...
    dplyr::mutate(`:=`(
      ...
      inverse_log_scaling_factor / .data$m
    )) %>%
    dplyr::select(-temp_scaling_factor, -inverse_log_scaling_factor, -.data$m)
