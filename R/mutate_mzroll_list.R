...
floor_peaks <- function(mzroll_list, log2_floor_value = 12) {
  ...
  missing_peaks <- tidyr::expand_grid(
    ...
    dplyr::anti_join(mzroll_list$peaks, by = c("groupId", "sampleId")) %>%
    tibble::as_tibble() %>%
  ...
  mzroll_list$peaks <- completed_peaks
...
#'   average
#' @param collapse_fxn function to use for collapse
#'
...
collapse_injections <- function(mzroll_list,
                                ...
                                collapse_fxn = "mean") {
  stopifnot(
    all(class(grouping_vars) %in% "character"),
    length(grouping_vars) >= 1
  )
  missing_grouping_vars <- setdiff(
    grouping_vars,
    colnames(mzroll_list$samples)
  )
  if (length(missing_grouping_vars) != 0) {
    stop(
      length(missing_grouping_vars),
      " variables were missing from the samples table: ",
      paste(missing_grouping_vars, collapse = ", ")
    )
  }

  stopifnot(
    all(class(peak_quant_vars) %in% "character"),
    length(peak_quant_vars) >= 1
  )
  missing_peak_quant_vars <- setdiff(
    peak_quant_vars,
    colnames(mzroll_list$peaks)
  )
  if (length(missing_peak_quant_vars) != 0) {
    stop(
      length(missing_peak_quant_vars),
      " variables were missing from the samples table: ",
      paste(missing_peak_quant_vars, collapse = ", ")
    )
  }

  stopifnot(class(collapse_fxn) == "character", length(collapse_fxn) == 1)
...
#'   }
#' @param quant_peak_varname variable in peaks to use for abundance
#' @param norm_peak_varname variable in peaks to add for normalized abundance
...
#'   a \code{norm_peak_varname} variable added to peaks
#'
#' @examples
#'
...
normalize_peaks <- function(mzroll_list,
  ...
  if (!(quant_peak_varname %in% colnames(mzroll_list$peaks))) {
    stop(
      "\"quant_peak_varname\":",
      quant_peak_varname,
      ", not present in peaks table"
    )
  }
...
#'
#' This normalization was reported in Kamphorst et al. 2015, see
...
normalize_peaks.median_polish <- function(mzroll_list,
  ...
  if (!is.na(log2_floor_value)) {
    ...
    normalization_peaks <- mzroll_list$peaks %>%
      dplyr::filter(
  ...
  if (any(is.na(mzroll_list$peaks[[quant_peak_varname]]))) {
    stop(
  ...
  if (!is.na(log2_floor_value)) {
    mzroll_list$peaks <- mzroll_list$peaks %>%
      ...
      # peaks starting at limit of detection are reset to log2_floor_value
      dplyr::bind_rows(
        mzroll_list$peaks %>%
          dplyr::filter(
            !!rlang::sym(quant_peak_varname) <= log2_floor_value + 0.001
      ...
      # peaks pushed below limit of detection are reset to log2_floor_value
      dplyr::mutate(
        !!rlang::sym(norm_peak_varname) :=
...
normalize_peaks.batch_center <- function(mzroll_list,
  ...
  centered_peaks <- mzroll_list$peaks %>%
    ...
    dplyr::select(
      !!!rlang::syms(c(colnames(mzroll_list$peaks), norm_peak_varname))
    )
  ...
  mzroll_list$peaks <- centered_refloor

  mzroll_list
}

#' @details compare each sample to the reference samples within the same batch
#'
...
normalize_peaks.reference_sample <- function(mzroll_list,
  ...
  annotated_peaks <- mzroll_list$peaks %>%
    dplyr::left_join(mzroll_list$samples %>%
      dplyr::select(
        sampleId,
  ...
  mzroll_list$peaks <- reference_refloor
...
normalize_peaks.reference_condition <- function(mzroll_list,
  ...
  references <- annotated_peaks %>%
    ...
    dplyr::summarize(.reference = mean(!!rlang::sym(quant_peak_varname))) %>%
    dplyr::ungroup()
  ...
  mzroll_list$peaks <- normalized_peaks
