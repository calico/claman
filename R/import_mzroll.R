...
#' 
#' @param peak_areas_path path to peak areas CSV file from metabolon results
#' @param chem_anno_path path to chemical annotations CSV file from metabolon results
#' @param metadata_path path to metadata CSV file from metabolon results
#' @param peak_floor_intensity value to use to replace NA peak measurements (missing values)
#' 
#' @export
process_metabolon <- function(peak_areas_path,
                              chem_anno_path,
                              metadata_path,
                              peak_floor_intensity = 4096
                              ) {
  
  checkmate::assertFileExists(peak_areas_path)
  checkmate::assertFileExists(chem_anno_path)
  checkmate::assertFileExists(metadata_path)
